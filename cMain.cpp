...
void cMain::OnMouse(wxMouseEvent& e) {

	std::cout << e.LeftDown() << std::endl;
	e.Skip();
}
...
void cMain::createButtonGrid() {
	...
	for (int iz = posFrom[1]; iz <= posTo[1]; iz++) {
		for (int ix = posFrom[0]; ix <= posTo[0]; ix++) {
			...
			button->Bind(wxEVT_ENTER_WINDOW, [this, pos](wxMouseEvent& e) { // dragging
				if (e.LeftIsDown()){
					toggleBlockState(pos);
				}
				});
			button->Bind(wxEVT_LEFT_DOWN, [this, pos](wxMouseEvent& e) { //Not the best method but works
				toggleBlockState(pos);
				});

			button->Bind(wxEVT_LEAVE_WINDOW, [this](wxMouseEvent& e) { // toggle module if clicked on module button
				this->DoCaptureMouse(); //works // breakes button dragging 
				this->DoReleaseMouse(); //getting this to work took me 4 hours 
				e.Skip();
				});
...
void cMain::toggleBlockState(GridPos* pos) {
	
	bool state = !pos->button->GetValue();
	pos->button->SetValue(state);
	if (state) {
